...
const sortDepositsByDate = (gotDeposits) => {
  return gotDeposits
    .sort((dep1, dep2) => Date.parse(dep1.date) - Date.parse(dep2.date))
    .reverse();
};
...
const getDepositItem = (depositObj) => {
  ...
  const date = moment.utc(depositObj.date).local().format(DATE_FORMAT);
...
const onMakeDepositFormSubmit = async (e, accessToken) => {
  ...
  const makeDepositSpinner = document.querySelector(
    `.${MAKE_DEPOSIT_SPINNER_CLASS}`
  );
  makeDepositSpinner.classList.remove(DISPLAY_NONE_CLASS);

  let makeDepositResult = null;
  try {
    makeDepositResult = await axios.post(
      POST_DEPOSIT_URL,
      {
        amount: amountValue,
      },
...
const deposits = async () => {
  ...
  if (gotDeposits.length === 0) {
    const noDepositsLabel = document.querySelector(
      `.${NO_DEPOSITS_LABEL_CLASS}`
    );
  ...
  makeDepositForm.addEventListener(
    "submit",
    async (e) => await onMakeDepositFormSubmit(e, accessToken)
  );
};
